$(function() {
  ...
  var Lib = {
    ...
    uploader: (function() {
      ...
      var s3Component = {
        ...
        initS3: function() {
          ...
          return new Promise(function(resolve, reject) {
            ...
            .done(function(response) {
              console.log('[API LOG] success! response:', response)
              if (response.status == 200) {
                that.s3Configs = response.result

                AWS.config.update({
                  credentials: {
                    accessKeyId: that.s3Configs.access_key_id,
                    secretAccessKey: that.s3Configs.secret_access_key,
                    sessionToken: that.s3Configs.session_token,
                  },
                })
                that.s3Client = new AWS.S3({
                  region: 'ap-northeast-1',
                  maxRetries: 0, // 指定しないと上がるまで上げ続けてしまい、タイムアウト時間までずっと待ち続けてしまうため
                })
                resolve(true)
              } else {
            ...
            .fail(function(e) {
              console.log('[API LOG] failed! response:', e)
              reject(e)
            })
          })
        },
         ...
         * @return {Promise<boolean>}
         */
        putObject: function(blob) {
          var that = this;

          return new Promise(function(resolve, reject) {
            that.s3Client.putObject(
              {
                Bucket: that.s3Configs.bucket,
                Key: that.s3Configs.key,
                Body: blob,
            ...
            that.s3Client.getObject(
              ...
              function (error, data) {
                if (error) {
                  ...
                  resolve(new Blob([data.Body], {type: data.ContentType}))
                }
              }
            );
          })
         ...
         */
        getOnetimeUri: function() {
          var that = this;

          return new Promise(function(resolve, reject) {
            $.ajax(
              '/api/v1/getS3OnetimeUri.php',
              {
                type: 'GET',
                cache: false,
                data: {
      ...
      makeErrorType = function() {
        var type = function(){
          Error.apply(this, arguments)
        };
        var superType = function(){}
        superType.prototype = Error.prototype
        type.prototype = new superType
        return type
      },
      RecordingError = makeErrorType(),
      UploadError = makeErrorType(),
      DownloadError = makeErrorType()
      ;
      ...
      return {
        /** @var */
        chanks: [],
        /** @var {object} Convert megabit/second to bit/second - https://www.convertunits.com/from/Mbps/to/bits/s */
        defaultOptions: {},
        /** @var */
        mediaRecorder: null,
        /** @type */
        ...
        startRecord: function (stream, options) {
          ...
          return new Promise(function (resolve, reject) {
            try {
              ...
              that.defaultOptions = Object.assign(
                {
                ...
                options,
                {
                  mimeType: Lib.uploader.tryGetSupportedMimeType(options.mimeType || 'video/webm;codecs=vp9'),
                }
              );
              that.mediaRecorder = new MediaRecorder(stream, that.defaultOptions);

              Object.entries({
                dataavailable: function (event) {
                  if (event.data && event.data.size > 0) {
                    that.chanks.push(event.data);
                  }
                },
                stop: function (event) {
                  var blob = new Blob(that.chanks, { type: that.chanks[0].type });
                  that.chanks = [];
                  that.mediaRecorder = null;

                  Lib.PromiseUtils.repeatUntilSuccessful(
                    function() {
                      return s3Component.putObject(blob)
                    },
                    {
                      interval: 1000,
                      retryMax: 10,
                  ...
                  .then(
                    function() {
                      Lib.PromiseUtils.repeatUntilSuccessful(
                        function() {
                          return s3Component.getObject()
                          //return s3Component.getOnetimeUri()
                        },
                        {
                          interval: 1000,
                          retryMax: 10,
                      ...
                      .then(
                        ...
                        function(e) {
                          reject(new DownloadError('動画URLの取得に失敗しました。(message=' + e.toString() + ')'));
                        }
                      )
                    },
                    function(e) {
                      reject(new UploadError('動画アップロードに失敗しました。(message=' + e.toString() + ')'));
                    }
                  )
                },
                error: function (event) {
                  ...
                  console.error(event);
                  reject(new RecordingError('メディアレコーダのエラーハンドルで致命的なエラーを取得しました。(event.error.name=' + event.error.name + ')'));
                },
              }).forEach(function (a) {
                that.mediaRecorder.addEventListener(a[0], a[1])
              })

              that.mediaRecorder.start(that.defaultOptions.timeslice); // timeslice
            } catch (e) {
              reject(new RecordingError('メディアレコーダの初期化中にエラーが発生しました。(message=' + e.toString() + ')'));
            }
          });
        },
        /**
            ...
            if (videoElement.src) {
              ...
              if (oldSrc) {
                if (Lib.ua.isFromAndroidOs() && Lib.ua.isChrome()) {
                  ...
                  Lib.URL.revokeObjectURL(oldSrc); // TODO:この処理が正しく行われる保証はない
                }
                oldSrc = '';
              }
            }
    ...
    PromiseUtils: {
       ...
       * @return {Promise<any>}
       */
      repeatUntilSuccessful: function (callback, options) {
        options = Object.assign({
          interval: 1000,
          retryMax: null,
          isAbort: function () { return false },
        }, (options || {}));
                    ...
                    setTimeout(
                      function() {
                        const promise = options.dispose
                          ? (options.dispose() || Promise.resolve(true))
      ...
      initMessageArea: function (settings) {
        ...
        return {
          ...
          showError: function (flags) {
            ...
            if (flags.recordingUpload) {
              $('#card-recording-upload').show();
            }
            if (flags.recordingDownload) {
              $('#card-recording-download').show();
            }
          ...
          showRecordingError: function () {
            this.showError({recording: true});
          },
          showRecordingUploadError: function () {
            this.showError({recordingUpload: true});
          },
          showRecordingDownloadError: function () {
            this.showError({recordingDownload: true});
          },
        };
      },
    };
        ...
        alert: {
          ...
          show: function (options) {
            ...
            return new Promise(function (resolve, reject) {
              ...
              if (options.bodyHTML) {
                modal.find('.modal-body .modal-text-area')
          ...
          show: function (options) {
            ...
            return new Promise(function (resolve, reject) {
              ...
              modal.on('shown.bs.modal', function (event) {
                ...
                Lib.htmlElementUtils.videoElement.setSrcObject(rawVideoPlayer, options.src)
                  .then(function() {
  ...
  if (cbResult !== Lib.ua.CheckBrowserResult.OK) {
    ...
    .finally(function() { (new Promise(function(resolve, reject) {
        ...
        var options = Lib.mediaDevices.storage.makeMediaStreamOptions({pc: {}, sp: {}});
          ...
          // var options = Lib.mediaDevices.storage.makeMediaStreamOptions({pc: {}, sp: {}});
          // let msResult = await Lib.mediaDevices.getMediaStream(true, /*true*/false, options)
          // // await sleep(1000)
          // view.videoPlayer.setVideoMediaInfo({show: true, icon: 'video-camera', status: 'LIVE', text: ''});
          // view.videoPlayer.attachStream(msResult.isEnableVideo, msResult.mediaStream);
          // resolve(true)
   ...
   * [カメラ・マイクチェック]ボタン
   */
  view.checkList.cameraCheck.onClick(function(event) {
    event.preventDefault();
    view.checkList.cameraCheck.blur();

    view.checkList.cameraCheck.setChecked(null);
    view.messageArea.hideAll();

    var process = {
      init: function () {
        // ボタンをすべて無効化
        view.deviceSelectBox.setEnabled(false);
        view.checkList.setEnabledAll(false);
    ...
    Lib.mediaDevices.getMediaStream(true, true, options).then(function (msResult) {
      ...
      var errorFlags = {};
      errorFlags.video = ! msResult.isEnableVideo;
      errorFlags.audio = ! msResult.isEnableAudio;
      if (Object.values(errorFlags).find(function (v) { return !!v; } )) {
        view.checkList.cameraCheck.setChecked(false);
        view.messageArea.showError(errorFlags);
        return Promise.reject(new Promise('カメラ・マイクがブロックされています'))
      } else {
        view.checkList.cameraCheck.setChecked(true);
        view.messageArea.showCameraSuccess();
        return Promise.resolve(true)
      }
    })
    //.catch(function () {/* エラーを握りつぶす */})
    .finally(function () {
      process.final();
    });
  });
   ...
   * [通知チェック]ボタン
   */
  view.checkList.notificationCheck.onClick(function(event) {
    event.preventDefault();
    view.checkList.notificationCheck.blur();

    view.checkList.notificationCheck.setChecked(null);
    view.messageArea.hideAll();
    ...
    new Promise(function (resolve, reject) {
      Lib.askForNPerm(function(notification) {
        if (! notification) {
          // 通知ブロックチェック　失敗
          view.reloadButton.show();

          view.checkList.notificationCheck.setChecked(false);
          view.messageArea.showNotificationError();

          reject(new Error('通知がブロックされています。'));
        } else {
          // 通知ブロックチェック　成功
          var
          params = {
            title: '通知確認',
            options: {
              body: '通知確認のテスト通知です。\nこの通知が確認出来たらこの通知をクリックしてください。',
              image: 'img/checker/notification_sample.png',
              requireInteraction: true,
            },
          },
          onSuccess = function() {
            view.checkList.notificationCheck.setChecked(true);
            view.messageArea.showNotificationSuccess();
          ...
          onError = function(event) {
            ...
            reject(new Error('通知チェックに失敗しました。'));
          },
          /**
           * 通常の通知の検証
           */
          taskDefault = function () {
             Lib.showNotification(params)
           ...
           * ServiceWorkerの通知の検証
           */
          taskByServiceWorker = function () {
            Lib.showNotificationByServiceWorker(params)
              .then(function(notification) {
                onSuccess()
              })
              .catch(function() {
   ...
   * [録画チェック]ボタン
   */
  view.checkList.recordingCheck.onClick(function(event) {
    event.preventDefault();
    view.checkList.recordingCheck.blur();

    view.checkList.recordingCheck.setChecked(null);
    view.messageArea.hideAll();
    ...
    Lib.mediaDevices.getMediaStream(true, true, options).then(function (msResult) {
      ...
      errorFlags.audio = ! msResult.isEnableAudio;
      if (Object.values(errorFlags).find(function (v) { return !!v; } )) {
        view.checkList.recordingCheck.setChecked(false);
        view.messageArea.showError(errorFlags);
        return Promise.reject(new Promise('カメラ・マイクがブロックされています'))
      } else {
        return new Promise(function (resolve, reject) {
          ...
          var timer = {
            ...
            SECONDS: 5.9, // 適当
            init: function() {
              view.videoPlayer.setVideoMediaInfo({show: true, icon: 'video-camera', status: '準備中', text: ('残り' + Math.floor(timer.SECONDS) + '秒')});
            },
            progress: function(sec) {
              view.videoPlayer.setVideoMediaInfo({show: true, icon: 'circle-o-notch', status: '録画中', text: ('残り' + Math.floor(sec) + '秒')});
            },
            start: function (callback) {
              return new Promise(function (resolve, reject) {
                var startTime = Date.now(),
                    run = function (msec) {
                      var v = Math.floor(timer.SECONDS - Math.floor(msec / 1000)),
                          sec = (v > 0) ? v : 0;
                ...
                timer.timerId = window.setInterval(
                  function () {
                    ...
                    // 5秒経過するまで待機/ちょっと適当
                    if (msec >= (timer.SECONDS * 1000)) {
                      timer.cancelIfNeeded();
          ...
          .then(function() {
            return Promise.all([
              Lib.uploader.startRecord(
                msResult.mediaStream,
                {
                  timeslice: 10000,
                }
              ),
              timer.start(function() {
                Lib.uploader.stopRecord()
              }),
            ])
          })
          .then(function (results) {
            ...
            //view.videoPlayer.detachStream();
            view.videoPlayer.setVideoMediaInfo({show: true, icon: 'pause-circle', status: '一時停止', text: ''});
            view.videoPlayer.pause();

            view.modals.videoPlayerModal.showSuccess({
              body: '再生ボタンを押して、正常に再生されれば録画チェックは完了です。',
              src: src,
            })
            .then(function (ok) {
              view.checkList.recordingCheck.setChecked(true);
              view.messageArea.showRecordingSuccess();
          ...
          .catch(function (error) {
            ...
            if (error instanceof Lib.uploader.UploadError) {
              view.checkList.recordingCheck.setChecked(false);
              view.messageArea.showRecordingUploadError()
              reject(error);
            } else if (error instanceof Lib.uploader.DownloadError) {
              view.checkList.recordingCheck.setChecked(false);
              view.messageArea.showRecordingDownloadError()
              reject(error);
            } else {
              view.checkList.recordingCheck.setChecked(false);
              view.messageArea.showRecordingError()
              reject(error);
            }
          })
        })
   ...
   * [リロード]ボタン
   */
  view.reloadButton.onClick(function(event) {
    event.preventDefault();
   ...
   * [ログイン]ボタン
   */
  view.loginButton.onClick(function(event) {
    event.preventDefault();

    if (Lib.parseUrlQuery().iframe) {
      window.parent.postMessage({request: 'login'}, '*');
    } else {
      view.modals.alert.showError({
        body: 'ログイン画面を開けませんでした。',
      })
    }
  ...
  /*
   * Mac + Safariの環境でのみ、チェッカー画面の通知チェックの欄にテキストを表示する
   */
  let browserInfo =  Lib.ua.getBrowserInfo()
  // ブラウザ　メッセージ
  if(browserInfo.name == "Safari" && browserInfo.os == "Mac") {
